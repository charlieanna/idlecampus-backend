# AUTO-GENERATED from golang_web_services.rb
puts "Creating Microlessons for Golang Web Services..."

module_var = CourseModule.find_by(slug: 'golang-web-services')

# === MICROLESSON 1: REST API Design Basics ===
...
# === MICROLESSON 2: Building HTTP Servers with net/http ===
...
# Exercise 2.1: Code (Hello handler)
Exercise.create!(
  micro_lesson: lesson_2,
  exercise_type: 'code',
  sequence_order: 1,
  exercise_data: {
    files: ['examples/go-web/main.go','examples/go-web/handlers/hello.go','examples/go-web/handlers/hello_test.go'],
    starter_code: <<~GO,
      // examples/go-web/main.go
      package main
      func main() {}
      // examples/go-web/handlers/hello.go
      package handlers
      import (
        "fmt"
        "net/http"
      )
      // Hello should write "Hello, World!" with status 200
    ...
    GO
    tests: {
      run: 'cd examples/go-web && (go mod init example.com/web >/dev/null 2>&1 || true) && go test ./... -count=1',
      visible: ['TestHello'],
      hidden: ['TestHelloMethodGet']
    },
    require_pass: true,
    hints: ['Write status 200 by default and write body "Hello, World!"', 'Use fmt.Fprintf(w, ...)']
  }
)
...
Exercise.create!(
  ...
  exercise_data: {
    ...
    options: ['http.ListenAndServe', 'http.ServeHTTP', 'http.Handle', 'http.ServerMux'],
    correct_answer: 0,
    explanation: 'http.ListenAndServe(":8080", handler) starts a server listening on the given address.',
    difficulty: 'easy'
  }
)

# === MICROLESSON 3: JSON Request/Response Handling ===
...
puts "✓ Created 3 microlessons for Golang Web Services"
