# AUTO-GENERATED from kubectl_learning_content.rb
puts "Creating Microlessons for Kubectl Learning Content..."

module_var = CourseModule.find_by(slug: 'kubectl-learning-content')
...
Exercise.create!(
  ...
  exercise_data: {
    ...
    require_pass: true,
    timeout_sec: 20,
    validation: { must_include: ['yes'], must_not_include: ['error:'] },
    hints: ['Use kubectl auth can-i ...', 'Try specifying a namespace if needed: -n default']
  }
)
...
Exercise.create!(
  ...
  exercise_data: {
    ...
    options: ['kubectl get clusterroles', 'kubectl get roles -A', 'kubectl describe clusterrolebinding', 'kubectl auth can-i --list'],
    correct_answer: 0,
    explanation: 'clusterroles are cluster-scoped roles listed via kubectl get clusterroles.',
    difficulty: 'easy',
    require_pass: true
  }
)

# Exercise 1.3: Code (Pod manifest)
Exercise.create!(
  micro_lesson: lesson_1,
  exercise_type: 'code',
  sequence_order: 3,
  exercise_data: {
    files: ['k8s/manifests/nginx-pod.yaml'],
    starter_code: <<~YAML,
      apiVersion: v1
      kind: Pod
      metadata:
    ...
    YAML
    tests: {
      run: 'kubectl apply -f k8s/manifests/nginx-pod.yaml && kubectl get pod nginx -o jsonpath="{.status.phase}"',
      visible: ['Pod becomes Running'],
      hidden: ['Label app=web present']
    },
    require_pass: true,
    hints: ['Ensure apiVersion/kind/metadata/spec are set', 'Container must expose port 80', 'Use a stable nginx image tag']
  }
)

# Exercise 1.4: Code (Deployment manifest)
Exercise.create!(
  micro_lesson: lesson_1,
  exercise_type: 'code',
  sequence_order: 4,
  exercise_data: {
    files: ['k8s/manifests/web-deploy.yaml'],
    starter_code: <<~YAML,
      apiVersion: apps/v1
      kind: Deployment
      metadata:
    ...
    YAML
    tests: {
      run: 'kubectl apply -f k8s/manifests/web-deploy.yaml && kubectl get deploy web -o jsonpath="{.status.availableReplicas}"',
      visible: ['Deployment available replicas == replicas'],
      hidden: ['Label selector matches template labels']
    },
    require_pass: true,
    hints: ['apps/v1 Deployment with selector matching template.labels', 'Set replicas >= 2', 'Expose port 80']
  }
)
...
puts "✓ Created 16 microlessons for Kubectl Learning Content"
