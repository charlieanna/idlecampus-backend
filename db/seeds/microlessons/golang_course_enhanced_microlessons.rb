# AUTO-GENERATED from golang_course_enhanced.rb
...
# Exercise 2.1: MCQ (exported vs unexported)
Exercise.create!(
  ...
  exercise_data: {
    require_pass: true,
    question: 'Which field is exported in Go structs?',
    options: ['Name', 'age', '_hidden', 'email'],
    correct_answer: 0,
    explanation: 'Fields starting with uppercase letters are exported (public).',
    difficulty: 'easy'
  }
)

# Exercise 2.2: Code (constructor & method)
Exercise.create!(
  micro_lesson: lesson_2,
  exercise_type: 'code',
  sequence_order: 2,
  exercise_data: {
    files: ['examples/go-structs/person/person.go','examples/go-structs/person/person_test.go'],
    starter_code: <<~GO,
      // examples/go-structs/person/person.go
      package person
      ...
      // New returns a Person with the given name and age.
    ...
    GO
    tests: {
      run: 'cd examples/go-structs && (go mod init example.com/structs >/dev/null 2>&1 || true) && go test ./... -count=1',
      visible: ['TestNew', 'TestIsAdult'],
      hidden: ['TestZero']
    },
    require_pass: true,
    hints: ['Return Person{Name:name, Age:age}', 'Compare p.Age >= 18']
  }
)
...
puts "✓ Created 34 microlessons for Go Basics"
