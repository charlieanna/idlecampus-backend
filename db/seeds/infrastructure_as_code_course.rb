...
puts "Creating Infrastructure as Code Course..."

iac_course = Course.find_or_create_by!(slug: 'infrastructure-as-code') do |course|
  course.title = 'Infrastructure as Code (IaC)'
  course.description = 'Automate infrastructure with Terraform, Ansible, and CloudFormation'
  course.difficulty_level = 'advanced'
  course.published = true
  course.sequence_order = 34
...
iac_fundamentals = CourseModule.find_or_create_by!(slug: 'iac-fundamentals', course: iac_course) do |mod|
  mod.title = 'IaC Fundamentals'
  mod.sequence_order = 1
  mod.estimated_minutes = 90
  mod.published = true
end

Lesson.find_or_create_by!(slug: 'terraform-fundamentals', course_module: iac_fundamentals) do |lesson|
  lesson.title = 'Infrastructure as Code with Terraform'
  lesson.sequence_order = 1
  lesson.estimated_minutes = 35
  lesson.lesson_type = 'video'
  lesson.published = true
  lesson.is_preview = true
  lesson.video_url = 'https://example.com/terraform-fundamentals'
...
# Module 2: Terraform Mastery
terraform_mastery = CourseModule.find_or_create_by!(slug: 'terraform-mastery', course: iac_course) do |mod|
  mod.title = 'Terraform Mastery'
  mod.sequence_order = 2
  mod.estimated_minutes = 160
  mod.published = true
end

Lesson.find_or_create_by!(slug: 'advanced-terraform-patterns', course_module: terraform_mastery) do |lesson|
  lesson.title = 'Advanced Terraform Patterns'
  lesson.sequence_order = 1
  lesson.estimated_minutes = 35
  lesson.lesson_type = 'video'
  lesson.published = true
  lesson.is_preview = false
  lesson.video_url = 'https://example.com/advanced-terraform'
  lesson.content = <<~MARKDOWN
...
# Module 3: Ansible Configuration Management
ansible_config = CourseModule.find_or_create_by!(slug: 'ansible-configuration', course: iac_course) do |mod|
  mod.title = 'Ansible Configuration Management'
  mod.sequence_order = 3
  mod.estimated_minutes = 140
  mod.published = true
end

Lesson.find_or_create_by!(slug: 'configuration-management', course_module: ansible_config) do |lesson|
  ...
  lesson.published = true
  lesson.is_preview = false
  lesson.video_url = 'https://example.com/configuration-management'
  lesson.content = <<~MARKDOWN
...
puts "  ✅ Created Infrastructure as Code course with comprehensive lessons\n"
puts "Created course: #{iac_course.title}"
puts "  Module 1: #{iac_fundamentals.title} - 1 lesson"
puts "  Module 2: #{terraform_mastery.title} - 1 lesson"
puts "  Module 3: #{ansible_config.title} - 1 lesson"
